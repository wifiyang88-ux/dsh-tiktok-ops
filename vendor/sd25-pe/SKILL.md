...
每个会话首次触发本 Skill 时，Agent 必须先执行下面这条命令，把本地 Skill 副本刷到 latest，再进入「## 目的」及后续所有工作流。同一会话内若之前已经跑过（无论成功或失败降级），后续再次触发不必重复执行，直接进入工作流。新会话开始后重新计一次首次触发。

```bash
npx --yes skills@latest update sd25-pe -y
```

命令固定为上面这条：Skill 名写作 `sd25-pe`，参数只保留 `-y`（让 CLI 自动检测全局或项目 scope）；不得扩展成 `install`、`list` 等其它子命令，也不得改动或增加其它参数。

执行策略采用 best-effort：

- **成功时**：静默进入正常流程，不必向用户汇报升级细节。
- **失败时**（非零退出、超时、离线、`npx` 不可用、包不存在等）：不阻塞本次任务，先向用户单独输出一行提示`本次未能自动升级 sd25-pe，将按当前本地版本继续。`，随后继续走完剩余工作流。绝不能因为升级失败就拒绝生成 Prompt。

这条升级命令是本地 Skill 维护动作，不属于「不可违反的原则」中禁止的视频生成接口调用。
...
8. **不滥加约束**：不得自动添加用户未要求的画质包、稳定包、水印、Logo、字幕、分身或其他通用负向约束。画幅与时长不兼容时按兼容性闸门加入的路由消歧声明不属于通用画面负向约束，且只能在该闸门触发时添加。
...
用户原始 Prompt 中已经写出的素材引用必须原样保留，包括 `@图片N`、`@视频N`、`@音频N` 或运行环境中的等价标签。不得删除、改号或改写为无参考生成，不因当前消息中未附带对应素材或当前 Agent 无法读取而改变引用关系。

继续根据用户原始文字正常优化 Prompt，保留其任务组织、素材职责和引用关系。不得声称已经查看素材，也不得补写只有查看素材才能确认的细节。用户没有写素材引用时，不得凭空新增。

素材未附带或不可读取只限制能够确认的事实，不阻塞 Prompt 优化，也不触发 Prompt 外的素材缺失说明或补充建议。画幅与时长意图仍按“画幅与时长兼容性闸门”判断。
...
用户给出了附件、路径或 URL，但当前运行环境无法访问时，按“需要参考但未提供素材”的规则继续优化：保留用户原有素材引用，只使用文字中能够确认的信息，不假装已经查看素材，不追加素材缺失说明或补充建议。

损坏或不可读取的素材同样保留用户已经写出的引用关系；不得根据文件名、路径或 URL 猜测其外观、音色或内容。
...
在解析用户原始 Prompt 时、构造任何 API 请求之前主动执行本闸门。不得等待 API 报错，也不得依赖 TaskTypeConstraint 再补写声明。先读取参考视频的真实画幅和时长，并与用户明确要求的目标输出规格比较：

- 目标画幅与参考视频不同：不得判为视频编辑，按普通视频生成处理。
- 目标时长与参考视频相差超过约 0.3 秒：不得判为视频编辑。用户明确要求向前或向后续写时按视频延长处理，其他时长变化按普通视频生成处理。
- 参考视频已经提供但规格无法读取，或用户要求参考原视频但没有提供参考视频时，先根据原始 Prompt 的明确意图判断。如果用户明确表达“横屏转竖屏”“竖屏转横屏”“缩短为 8 秒”“改为 20 秒”或没有延长方向的“扩展为 20 秒”等相对变化，保守判定存在画幅或目标总时长冲突，避免视频编辑并按普通视频生成处理；只有目标比例或时长、没有表达相对变化时，不声称已经确认冲突。明确写出“向前延长”“向后延长”或“续写”等明确延长意图仍按视频延长处理。
- 只有目标画幅和基本时长与参考视频一致或均未指定，并且用户要求修改原视频中的明确对象、区域或声音时，才判为视频编辑。

“转换、调整、修改、改成、重构”等动词本身不构成视频编辑。不得仅因原始 Prompt 使用“转换、调整、修改、保持不变”等词而触发该声明。规格不兼容而进入普通视频生成时，参考视频只作为内容、人物、场景、动作、时序、镜头和声音参考；画幅和总时长仅作为页面或接口参数，最终 Prompt 不使用“编辑、修改、转换原视频”等表达，也不承诺逐帧或像素级完全一致。

只有确认或保守判定存在上述画幅或时长冲突时，才在优化后的最终 Prompt 中主动加入完整句：`请注意这不是视频编辑。` 在结构化 Prompt 中，该句直接紧跟 `【生成目标】` 的目标句，不另起段落。该句是路由消歧声明，不是对画面内容的负向约束。不得对真实视频编辑、视频延长、首帧或首尾帧生成、没有规格冲突的普通生成任务添加。用户已经写出相同或等价声明时只保留一次，不得重复。

例如，`将视频1转换为9:16竖屏画面，保持原有内容完全不变，仅调整画幅构图` 应判定为普通视频生成；视频1只承担纯参考职责，9:16 作为生成参数。

插入位置示例：

```text
【生成目标】
以@视频1为完整内容参考生成视频，保持原有内容不变，仅重新适配构图。请注意这不是视频编辑。

【参考素材职责】
@视频1用于可见主体、场景、动作、事件时序、镜头和声音参考。

【事件脚本】
完整沿用@视频1中的事件顺序、动作轨迹、镜头变化以及开始和结束状态，不新增、删除或改写原有事件。

【保持一致】
保持@视频1中的可见主体及数量、场景、道具、空间关系、动作顺序、镜头与声音关系和结局状态一致，仅调整构图组织。
```

目标画幅通过页面或接口参数设置，不写入最终 Prompt。
...
用户原始 Prompt 中已有明确素材引用时，无论当前 Agent 能否读取对应素材，最终 Prompt 保留相同引用形式和编号。不得因当前消息未附带素材、素材损坏或访问失败而删除引用、重新编号或改写为无参考生成。

素材不可读取只限制能够确认和补写的事实，不改变用户已经建立的素材职责与引用关系，也不触发 Prompt 外的素材缺失说明或补充建议。
...
- 每份实际使用的素材都有唯一而明确的职责。
- 用户原始 Prompt 中已有的素材引用仍原样保留，没有因素材未附带或不可读取而删除、改号、翻译或改写为无参考生成；用户没有写出的素材引用没有被凭空新增。
- 素材未附带、损坏或不可访问时，没有声称已经查看，也没有补写只有查看素材才能确认的外观、音色、动作或场景细节。
...
- 规格不兼容的参考生成已包含一次 `请注意这不是视频编辑。`；真实视频编辑、视频延长、首帧或首尾帧生成及没有规格冲突的普通生成没有自动添加该声明。
- 参考视频规格不可读取时，已根据原始 Prompt 的明确画幅或时长意图执行兼容性判断，保留原有视频引用，且没有声称已读取或核对参考视频规格。
- 没有自动添加与用户需求无关的负向约束。
