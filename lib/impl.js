 ...
 */
/**
 * 登录入口统一走 TikTok Studio。
 *
 * 以前深链到 `/login/phone-or-email/email`，那会**强制**展示邮箱密码表单——
 * 而 tiktokstudio 才是发布视频用的 Web 端：在它那儿登录，拿到的登录态正好就是后面
 * 发布 / 采集要复用的那个。而且登录页的**默认面板就是二维码**，
 * 只有备注名+用户名、没有密码的账号（只能扫码）也能直接用。
 */
const TIKTOK_STUDIO_URL = 'https://www.tiktok.com/tiktokstudio';

/** 账号能不能走「账号密码自动登录」——只配了备注名与用户名时不能，只能扫码。 */
const canPasswordLogin = (account) => String(account?.password ?? '').trim() !== '';

/**
 * 按可见文字点一个元素。
 *
 * TikTok 登录页的几个入口都是纯文字块，没有稳定的选择器；用 innerText 精确匹配
 * 比猜 class 稳。返回 'clicked' / 'not-found' / 'error'，让调用方决定要不要重试。
 */
async function clickByText(settings, session, text) {
  return abEval(
    settings,
    session,
    `(() => {
      const target = ${JSON.stringify(text)};
      const hit = [...document.querySelectorAll('div,button,a,span,li')]
        .filter((n) => (n.innerText || '').trim() === target)
        .pop();
...
async function tiktokLogin(settings, account) {
  ...
  const withPassword = canPasswordLogin(account);
  ...
  // ---- 只有配了密码才尝试自动登录 ----
  // 没密码的账号（只填了备注名 + 用户名）只能扫码，硬试表单毫无意义，
  // 以前还会在路由那一层直接抛「无法自动登录」，把扫码这条路彻底堵死。
  if (withPassword) {
    await ab(settings, session, ['open', TIKTOK_STUDIO_URL], { timeoutMs: 120000 }).catch(() => {});
    // 登录页默认是二维码面板，要先切到「手机号/电子邮箱/用户名登录」才露得出表单
    await clickByText(settings, session, '使用手机号码/电子邮箱/用户名登录');
  ...
  // ---- 兜底：重建成有头窗口，让人手动完成 ----
  // 刻意**不**切任何面板：登录页默认就是二维码（见参考图），无密码的账号直接扫即可；
  // 有密码的账号如果自动登录被验证码拦下，也可以改用扫码，比找密码更快。
  await ab(settings, session, ['close'], { timeoutMs: 60000 }).catch(() => {});
  await ab(settings, session, ['--headed', 'open', TIKTOK_STUDIO_URL], { timeoutMs: 120000 }).catch(() => {});

  return {
    status: 'need_manual',
    message: withPassword
      ? '自动登录未通过（通常是验证码或二次验证）。已在有头窗口打开 TikTok Studio 登录页，请手动完成——也可以直接用手机 App 扫码登录。之后会复用该会话。'
      : '该账号只配置了备注名与登录用户名，没有密码，无法自动登录。已在有头窗口打开 TikTok Studio 登录页，页面上默认展示的就是登录二维码：请用手机 TikTok App 扫码完成登录，之后会复用该会话。',
  };
}
...
function buildRoutes(ctx) {
  ...
  routes.push(
    jsonRoute('/account/login', async (body) => {
      ...
      // 这里**不再**因为「没有密码」就拒绝：只配了备注名 + 用户名的账号
      // 本来就只能扫码登录，tiktokLogin 会开一个有头窗口让人扫。
      // 以前那道闸门直接把扫码这条路堵死了。
