...
const PROMPT_GUIDANCE = [
  ...
  '并在提示词里明确写出「使用英语」（官方指南指出：不写明时英语台词容易被生成成中文）。',
  '只有任务标了其它市场时才改。',
].join('\n');
  ...
  routes.push({
    ...
    handler: (req, res) => {
      ...
      writeJson(res, 200, {
        ...
        guidance: { ...guidanceState, mentionsSkill: /sd25-pe/.test(PROMPT_GUIDANCE) },
        dispatch: {
...
async function registerTools(ctx) {
  ...
  for (const tool of tools) {
    try {
      ...
      ctx.logger?.warn?.(`tiktok-ops: 工具 ${tool?.name} 注册失败：${error}`);
    }
  }
  ctx.logger?.info?.(`tiktok-ops: 已注册 ${tools.length} 个模型工具`);
}
...
export async function apply(ctx) {
  ...
  const registerGuidance = (target) => {
    try {
      disposers.push(target.systemPrompt.section({ name: 'plugin:tiktok-ops', order: 220, text: PROMPT_GUIDANCE }));
