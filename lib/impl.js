...
export async function apply(ctx) {
  ...
  // 先探一下 sd25-pe 在不在，再决定注入哪一版说明。
  // 它常常不在（那是 workspace 级 skill，git 装进来的插件在别人机器上没有），
  // 所以探测结果要记进 /diag，别让「agent 被要求加载不存在的东西」再次静默发生。
  const hasSd25Pe = await detectSkill(ctx, 'sd25-pe');
