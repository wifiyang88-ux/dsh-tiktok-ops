			...
			return h(
				...
				h(
					...
					h(
						...
						h(Field, { label: "密码" }, h("input", { style: T.input, type: "password", value: account.password, onChange: set("password"), placeholder: "留空则只能扫码登录" }))
					),
					h(Btn, {
						primary: true,
						...
						: state.accounts.map((a) =>
								h(
									...
									h(
										...
										// 只填了备注名 + 用户名 = 没有密码 = 只能扫码登录。
										// 这件事必须在卡片上看得见，否则用户会以为「登录」按钮坏了。
										a.hasPassword
											? h("span", { style: { fontSize: "11px", ...T.muted } }, "已保存密码")
											: h("span", { style: { fontSize: "11px", color: "#d9a441" } }, "未保存密码 · 只能扫码登录")
									),
